...
//			List<CarMonitorVO> queryAddMonitorCar = MongoUtils.queryMonitorCars(query);
	 ...
	 * @return
	 */
	@RequestMapping(value = "/queryAddMonitorCarToMongo.do", method = RequestMethod.GET)
	@ResponseBody
	public Map<String, Object> queryAddMonitorCarToMongo(String accessToken, Integer userId,
			String keys){
		Map<String, Object> model = new HashMap<String, Object>();
		Map<String, Object> params = new HashMap<String, Object>();
		try {
			...
			CommonUtils.checkAccessToken(accessToken, model);
			CommonUtils.checkParams("userId", userId, model);
			CommonUtils.checkParams("keys", keys, model);
			CarMonitorQuery query = new CarMonitorQuery();
			query.setUserId(userId);
...
//			List<CarMonitorVO> queryAddMonitorCar = monitorCarService.queryAddMonitorCar(query);
			List<CarMonitorVO> queryAddMonitorCar = MongoUtils.queryMonitorCars(query);
//			int len = queryAddMonitorCar.size();
			
				//循环遍历，将符合条件信息移除
				for (Iterator<CarMonitorVO> iterator = queryAddMonitorCar.iterator(); iterator
						.hasNext();) {
					CarMonitorVO carMonitorVO = (CarMonitorVO) iterator.next();
					params.put("userId", userId);
					params.put("carId", carMonitorVO.getCarId());
					//查询该车辆是否被该用户监控
					Integer queryCarMontiorCount = carMonitorService.queryCarMontiorCount(params);
					if(null != queryCarMontiorCount && queryCarMontiorCount>0){
						iterator.remove();
					}
					if(null != carMonitorVO.getCarTypeFirstCode()){
						Integer carTypeFirstCode=carMonitorVO.getCarTypeFirstCode();
